...
def venn(data, names=None, fill=NUMBER, show_names=True, save_name=None, show_plot=True, **kwds):
	"""
	data: a list
	names: names of groups in data
	fill = [NUMBER | PERCENT | LOGIC], fill with number, percent, logic label, or a combination
	show_names = [True|False]
	save_name = [None|some filename]
	...
	if len(data) == 2:
		venn2(data, names, fill, show_names, save_name, show_plot, **kwds)
	elif len(data) == 3:
		venn3(data, names, fill, show_names, save_name, show_plot, **kwds)
	elif len(data) == 4:
		venn4(data, names, fill, show_names, save_name, show_plot, **kwds)
	else:
		raise NotImplementedError("Currently only 2-4 sets venn diagrams are supported")
...
def get_labels(data, fill=NUMBER):
	...
	for k in set_collections:
		label = []
		if fill & NUMBER:
			label.append(len(set_collections[k]))
		if fill & PERCENT:
			label.append('{:.2f}'.format(len(set_collections[k]) / totalCount))
		if fill & LOGIC:
			labels.append(k)
		labels[k] = '\n'.join(map(str, label))
...
def venn2(data=None, names=None, fill=NUMBER, show_names=True, save_name=None, show_plot=True, **kwds):
...
def venn3(data=None, names=None, fill=NUMBER, show_names=True, save_name=None, show_plot=True, **kwds):
...
def venn4(data=None, names=None, fill=NUMBER, show_names=True, save_name=None, show_plot=True, **kwds):
...
def test():
	...
	# venn3()
	venn([range(10), range(5,15), range(3,8)], ["aaaa", "bbbb", "cccc"], fill=NUMBER | PERCENT, show_names=False)
	# venn2()
